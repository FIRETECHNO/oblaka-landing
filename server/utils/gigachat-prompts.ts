import { oblakaCocktails } from "./cocktails"

export function generateSystemPrompt(): string {
  const menuLines = oblakaCocktails.map(cocktail => {
    return `${cocktail._id} | ${cocktail.name} | ${cocktail.ingredients} | ${cocktail.description}`
  }).join('\n')

  return `Ты — бармен в премиум-баре. У тебя есть меню из ${oblakaCocktails.length} напитков. Каждый напиток имеет уникальный числовой идентификатор _id (от 1 до ${oblakaCocktails.length}), категорию, описание вкуса, крепость и объём.
